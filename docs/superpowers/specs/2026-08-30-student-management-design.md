...
시험·숙제 줄은 항목 하나가 한 줄에 들어간다. 교재명이 중간에서 끊기면 어느 책인지 알아보기 어렵기 때문이다. 시험 점수는 굵게 낸다 — 그 줄에서 학부모가 가장 먼저 찾는 값이다. 흔히 쓰는 길이의 교재명은 좁은 휴대폰(375px)에서도 한 줄에 들어가고, 그보다 긴 이름은 그 묶음만 옆으로 밀린다. 페이지 전체가 가로로 움직이지 않도록 `dd`에 `min-width:0`을 둔다.
