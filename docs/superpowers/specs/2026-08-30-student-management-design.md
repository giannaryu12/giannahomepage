...
타임라인에는 **진도나 시험이 하나라도 적힌 날만** 낸다(`lib/shape.js`의 `hasParentContent_`). 출결만 찍고 저장한 날이나 날짜만 열었다가 그대로 저장된 날은 학부모가 열어 봐야 읽을 것이 없다. 교재는 직전 수업에서 자동으로 채워지므로 근거가 되지 못하고, 시험은 점수와 만점이 함께 있어야 기록으로 본다(0점은 기록이다).

거르기는 **요약 통계를 계산한 뒤에** 한다. 결석한 날은 진도도 시험도 없어 목록에서 빠지는데, 빠진 채로 세면 결석이 지워져 출석률이 100%가 되어 버린다. 바꿔 말하면 결석한 날은 목록에 줄이 생기지 않고 출석률에만 반영된다.
