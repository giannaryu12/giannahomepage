...
| vocabNext … etcNext | 영역별 다음 과제 (진도와 같은 다섯 영역) |
| vocabTestScore / vocabTestMax | 단어 시험 |
| listeningTestScore / listeningTestMax | 듣기 시험 |
| homeworkStatus | `제출` \| `부분제출` \| `미제출` \| `해당없음` |
| homeworkLevel | `상` \| `중` \| `하` \| `` |
| testName / testScore / testMax | 시험이 단어·듣기로 나뉘기 전의 기록. 새로 쓰지 않고 보존만 한다 |
| attendance | `출석` \| `지각` \| `결석` \| `보강` |
| nextHomework | 영역이 나뉘기 전의 다음 과제. 새로 쓰지 않고 보존만 한다 |
| comment | 선생님 코멘트 |
...
3. 학생 행을 누르면 그 학생 한 명의 기록 입력 화면으로 이동한다. 진도(단어·독해·문법·듣기·기타 다섯 영역, 영역마다 교재와 진도 두 칸) · 출결(4버튼) · 과제(4버튼) · 완성도(상중하) · 시험(단어·듣기 각각 점수/만점) · 다음 과제(진도와 같은 다섯 영역) · 코멘트를 입력하고 「저장」을 누르면 그 자리에서 `admin.saveBatch`를 학생 1명 분(`records: [한 건]`)만 호출한다
...
교재는 그 학생의 직전 기록에서 자동으로 채운다(`admin.roster`가 학생마다 `lastBooks`를 함께 내려준다). 아직 기록이 없는 날짜를 열 때만 채우고, 이미 저장된 기록에는 채우지 않는다 — 그날 하지 않은 영역에 교재만 붙으면 하지도 않은 수업이 기록에 남는다. 학부모 화면에서 영역을 보일지 말지도 교재가 아니라 **진도**가 적혔는지로 판단한다.
