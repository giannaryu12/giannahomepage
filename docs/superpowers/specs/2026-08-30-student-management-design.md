...
시험과 숙제는 같은 꼴이다: 앞줄에 `<영역> <값>`, 그 아래 줄에 교재명을 들여 쓴다. 값(시험은 점수, 숙제는 범위)이 학부모가 가장 먼저 찾는 것이라 교재명이 길다고 화면 밖으로 밀리면 안 된다. 시험 점수만 굵게 낸다 — 숙제 범위는 문장이라 굵게 하면 줄 전체가 무거워진다. 교재명이 비어 있으면 아랫줄 자체가 생기지 않는다.
