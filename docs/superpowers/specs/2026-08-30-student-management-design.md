...
3. 학생 행을 누르면 그 학생 한 명의 기록 입력 화면으로 이동한다. 진도(단어·독해·문법·듣기·기타 다섯 영역, 영역마다 교재와 진도 두 칸) · 출결(4버튼) · 과제(4버튼) · 완성도(상중하) · 시험명/점수/만점 · 다음 과제 · 코멘트를 입력하고 「저장」을 누르면 그 자리에서 `admin.saveBatch`를 학생 1명 분(`records: [한 건]`)만 호출한다
