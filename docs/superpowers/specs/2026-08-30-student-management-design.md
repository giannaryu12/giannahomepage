...
──────────────────────────────
(출석률 96%) (숙제 88%)                전체 요약 카드 (두 줄 고정, 고리)
...
요약은 **그 학생의 전체 기록**으로 낸다. `parent.load`에 `monthKey`를 주면 그 달만 세지만, 화면은 주지 않는다. 이번 달로 자르면 달이 바뀌는 1일마다 카드가 전부 `기록 없음`이 되는데, 아래 수업 기록에는 지난달 수업이 그대로 보이므로 오류로 읽힌다.
