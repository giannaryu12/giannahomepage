...
| vocabTestBook / vocabTestScore / vocabTestMax | 단어 시험 교재 · 점수 · 만점 |
| listeningTestBook / listeningTestScore / listeningTestMax | 듣기 시험 교재 · 점수 · 만점 |
| homeworkStatus | `제출` \| `부분제출` \| `미제출` \| `해당없음` |
| homeworkLevel | `상` \| `중` \| `하` \| `` |
| testName / testScore / testMax | 시험이 단어·듣기로 나뉘기 전의 기록. 새로 쓰지 않고 보존만 한다 |
...
[단어 90점] [듣기 74점]
──────────────────────────────
   성적 추이 (인라인 SVG, 단어·듣기 두 선)
──────────────────────────────
▸ 10/12  단어 Day 12 · 독해 Unit 7
         출석 · 과제 제출(상) · 시험 단어 18/20, 듣기 8/10
         숙제 단어 보카 워크북 · Day 14
...
성적 추이 차트는 외부 라이브러리 없이 인라인 SVG로 그린다. 데이터 포인트가 2개 미만이면 차트를 숨기고 안내 문구를 보여준다. 단어와 듣기를 색이 다른 두 선으로 그리고 범례를 붙인다. 두 시험의 날짜가 서로 달라도 같은 가로축 위에 놓는다 — 축이 다르면 비교가 성립하지 않는다. 점이 하나뿐인 선도 점은 찍는다(선만 그리면 화면에서 사라진다).

시험이 단어·듣기로 나뉘기 전에 쌓인 점수는 새 기록이 하나도 없을 때 한 선으로 되살린다. 요약 카드에도 그 값이 있을 때만 `이전 시험 평균`을 덧붙인다 — 늘 띄우면 옛 기록이 없는 학생에게 뜻 없는 칸이 남는다.
...
3. 학생 행을 누르면 그 학생 한 명의 기록 입력 화면으로 이동한다. 진도(단어·독해·문법·듣기·기타 다섯 영역, 영역마다 교재와 진도 두 칸) · 출결(4버튼) · 과제(4버튼) · 완성도(상중하) · 시험(단어·듣기 각각 교재/점수/만점) · 숙제(진도와 같은 다섯 영역, 영역마다 교재와 내용) · 코멘트를 입력하고 「저장」을 누르면 그 자리에서 `admin.saveBatch`를 학생 1명 분(`records: [한 건]`)만 호출한다
...
교재 칸은 진도·숙제·시험에 각각 따로 있고(합쳐서 12칸), 모두 그 학생의 직전 기록에서 자동으로 채운다(`admin.roster`가 학생마다 `lastBooks`를 함께 내려준다. 키는 필드 이름 그대로다). 아직 기록이 없는 날짜를 열 때만 채우고, 이미 저장된 기록에는 채우지 않는다 — 그날 하지 않은 영역에 교재만 붙으면 하지도 않은 수업이 기록에 남는다.
